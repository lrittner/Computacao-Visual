...
class ObjLoader:
    ...
    def __init__(self, file):
        ...
        for name, material in self.scene.materials.items():
            ...
            if(len(material.vertices) == 0):
                print('material_name =', name, '- No vertice data')
                continue
            ...
            if(self.__objectInfo[name].vertexFormat == "T2F_N3F_V3F"):
            ...
            elif(self.__objectInfo[name].vertexFormat == "T2F_C3F_V3F"):
                ...
                self.__objectInfo[name].colors = np.zeros(self.__objectInfo[name].numVertices * 3, dtype=np.float32)
                self.__objectInfo[name].colors[0::3] = vertices[2::8]
                self.__objectInfo[name].colors[1::3] = vertices[3::8]
                self.__objectInfo[name].colors[2::3] = vertices[4::8]
    ...
    def getVertexColors(self, item_name):
    
        return self.__objectInfo[item_name].colors
    ...
    def hasVertexColors(self, item_name):
        
        return (len(self.__objectInfo[item_name].colors) > 0)
